import React from 'react'
import { useThemeStore } from '../store/useThemeStore'
import { PaletteIcon } from 'lucide-react'
import { THEMES } from '../constants'

const ThemeSelector = () => {
    const {theme , setTheme} = useThemeStore()
  return (
    <div className='dropdown dropdown-end'>
        <button className='btn btn-ghost btn-circle' tabIndex={0}>
        <PaletteIcon className='size-5'/>
        </button>

        <div tabIndex={0} className='dropdown-content mt-2 p-1 shadow-2xl bg-base-200 backdrop-blur-lg rounded-2xl w-56 border border-base-content/10 max-h-80 overflow-y-auto'>
            <div className='space-y-1'>
                {
                    THEMES.map((themeOption)=>(
                        <button className={`w-full px-4 py-3 rounded-xl flex items-center gap-3 transition-colors ${theme===themeOption.name?'bg-primary/10 text-primary':"hover:bg-base-content/5"}`} key={themeOption.name} onClick={()=>setTheme(themeOption.name)}>

                        <PaletteIcon className='size-4 text-primary'/>
                        <span className='text-sm font-medium'>{themeOption.label}</span>
                        {themeOption.colors.map((color,i)=>(
                            <span className='size-2 rounded-full' key={i} style={{backgroundColor:color}}></span>
                        ))}
                        </button>
                    ))
                }
            </div>
        </div>

    </div>
  )
}

export default ThemeSelector